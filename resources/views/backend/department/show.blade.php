    ...
    <div class="py-12">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                <div class="item-card flex flex-row gap-y-10 justify-between items-center">
                    <div class="flex flex-row items-center gap-x-3">
                        <div class="flex flex-col">
                            <h3 class="text-indigo-950 text-xl font-bold">{{ $department->name }}</h3>
                            <p class="text-slate-500 text-sm">{{ $department->code }}</p>
                        </div>
                    </div>
                    <div class="flex flex-col">
                        <p class="text-slate-500 text-sm">Position</p>
                        <h3 class="text-indigo-950 text-xl font-bold">{{ $department->position->count() }}</h3>
                    </div>
                    <div class="flex flex-row items-center gap-x-3">
                ...
                <div class="flex flex-row justify-between items-center">
                    <div class="flex flex-col">
                        <h3 class="text-indigo-950 text-xl font-bold">Position</h3>
                        <p class="text-slate-500 text-sm">{{ $department->position->count() }} Total Positions</p>
                    </div>
                    <a href="{{ route('department.create.position',$department) }}" class="font-bold py-2 px-4 bg-indigo-700 hover:bg-indigo-400 text-white rounded-full">
                        Add New Position
                    </a>
                </div>

                @foreach($department->position as $pos)
                <div class="item-card flex flex-row gap-y-10 justify-between items-center">
                    <div class="flex flex-row items-center gap-x-3">
                        <div class="flex flex-col">
                            <h3 class="text-indigo-950 text-xl font-bold">{{ $pos->name }}</h3>
                            <p class="text-slate-500 text-sm">{{ $pos->code }}</p>
                        </div>
                    </div>

                    
                    <div class="flex flex-row items-center gap-x-3">
                        <a href="{{ route('position.edit',$pos) }}" class="font-bold py-2 px-4 bg-indigo-700 text-white rounded-full">
                            Edit
                        </a>
                        <form action="{{ route('position.destroy',$pos) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="font-bold py-2 px-4 bg-red-700 text-white rounded-full">
