<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-row justify-between items-center">
            ...
            @role('superadmin')
                <a href="{{ route('question.create') }}" class="font-bold py-2 px-4 bg-indigo-700 hover:bg-indigo-400 text-white rounded-full">
                    Add New
                </a>
            @endrole
        </div>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                <div class="flex flex-col md:flex-row justify-end items-center mb-6">
                    ...
                    <form method="GET" action="{{ route('question.index') }}" class="flex w-full md:w-auto">
                ...
                <table class="border-collapse table-auto w-full whitespace-no-wrap bg-white table-striped relative">
                    ...
                    <tbody id="users-table-body">
                        @forelse ($questions as $question)
                            <tr>
                                <td class="border-b border-gray-200 px-6 py-4">{{ $question->description }}</td>
                                <td class="border-b border-gray-200 px-6 py-4">{{ $question->difficult }}</td>
                                <td class="border-b border-gray-200 px-6 py-4">
                                    <a href="{{ route('question.show', $question) }}" class="font-bold py-2 px-4 bg-indigo-700 hover:bg-indigo-400 text-white rounded-lg">View</a>
                                </td>
                            <tr>
                        @empty
                        <tr>
