                ...
                <form method="POST" action="{{ route('schedule.update',$schedule) }}" enctype="multipart/form-data">
                    ...
                    <div class="mt-4">
                        ...
                        <select name="job_vacancy_id" id="job_vacancy_id" class="rounded-lg pl-3 w-full border border-slate-300">
                            ...
                            @foreach ($jobs as $job)
                                <option value="{{ $job->id }}" {{ $schedule->job_vacancy_id == $job->id  ? 'selected':''}}>{{ $job->code }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mt-4">
                        ...
                        <x-text-input id="date" class="block mt-1 w-full" type="date" name="date" value="{{ $schedule->date }}" required autofocus autocomplete="date" />
