<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-row justify-between items-center">
            ...
            @role('superadmin')
                <a href="{{ route('question.show',$choice->question) }}" class="font-bold py-2 px-4 bg-indigo-700 text-white rounded-full">
                    Back
                </a>
            @endrole
        </div>
    ...
    <div class="py-12">
                ...
                <div class="item-card flex flex-row gap-y-10 justify-between items-center">
                    <div class="flex flex-row items-center gap-x-3">
                        <div class="flex flex-col">
                            <h3 class="text-indigo-950 text-xl font-bold">{{ $choice->question->description }}</h3>
                            <p class="text-slate-500 text-sm">{{ $choice->question->difficult }}</p>
                        </div>
                    </div>
                    <div class="flex flex-col">
                        {{-- <p class="text-slate-500 text-sm">Image</p> --}}
                        @if ($choice->question->image)
                            <img src="" alt="" class="rounded-2xl object-cover w-[200px] h-[150px]">
                        @endif
                    </div>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 mt-5">
            @if($errors->any())
                ...
                @endif
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                <form method="POST" action="{{ route('choice.update',$choice) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="mt-4">
                        <x-input-label for="label" :value="__('Label')" />
                        <x-text-input id="label" class="block mt-1 w-full" type="text" name="label" value="{{ $choice->label }}" required autofocus autocomplete="label" />
                    ...
                    <div class="mt-4">
                        ...
                        <textarea id="answerText" name="answerText" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" rows="4">{{ $choice->answerText }}</textarea>
                    ...
                    <div class="mt-4">
                        ...
                        @if ($choice->answerImage)
                            <img src="{{ Storage::url($choice->answerImage) }}" alt="" class="rounded-2xl mt-4 object-cover w-[200px] h-[150px]">
                        @endif
                    </div>
                    ...
                    <div class="mt-4 flex items-center">
                        <input 
                            ...
                            name="is_correct" 
                            class="mr-2" 
                            style="width: 24px; height: 24px;" {{ $choice->is_correct == true ? 'checked':'' }}
                        />
